#!/usr/bin/env node
const fs=require('fs'),path=require('path');
function env(){ if(process.env.DATABASE_PUBLIC_URL||process.env.DATABASE_URL)return;
  for(const p of [path.join(__dirname,'backend','.env'),path.join(__dirname,'.env')]) if(fs.existsSync(p))
    for(const l of fs.readFileSync(p,'utf8').split(/\r?\n/)){const m=l.match(/^\s*DATABASE_URL\s*=\s*(.*)\s*$/);if(m){process.env.DATABASE_URL=m[1].replace(/^["']|["']$/g,'');return;}}
}
const SPRINT='0c83f962-52b6-4403-a7d4-4915b541fa0f';
(async()=>{ env();
  let Client; try{({Client}=require('pg'));}catch(_){({Client}=require(path.join(__dirname,'backend','node_modules','pg')));}
  let url=(process.env.DATABASE_PUBLIC_URL||process.env.DATABASE_URL).replace(/([?&])(sslmode|ssl|channel_binding)=[^&]*/gi,'$1').replace(/[?&]+$/,'').replace(/\?&+/,'?').replace(/&&+/g,'&');
  const c=new Client({connectionString:url,ssl:{rejectUnauthorized:false}});
  await c.connect();
  const r=await c.query(`
    SELECT c.titulo,
           (SELECT string_agg(p.nome,', ') FROM cards_responsaveis cr JOIN pessoas_acesso p ON p.id=cr.pessoa_id WHERE cr.card_id=c.id) resp,
           to_char(c.data_inicio,'DD/MM') di, to_char(c.data_prazo,'DD/MM') df
      FROM cards c WHERE c.sprint_id=$1 AND c.arquivado_em IS NULL ORDER BY c.ordem`,[SPRINT]);
  for(const x of r.rows) console.log(`- ${x.titulo.slice(0,40).padEnd(40)} | ${(x.resp||'(sem resp)').padEnd(34)} | ${x.di||'—'}→${x.df||'—'}`);
  const t=await c.query(`SELECT count(*)::int n, count(data_prazo)::int comdata, count(*) FILTER (WHERE EXISTS(SELECT 1 FROM cards_responsaveis cr WHERE cr.card_id=cards.id))::int comresp FROM cards WHERE sprint_id=$1 AND arquivado_em IS NULL`,[SPRINT]);
  console.log(`\nTotal ${t.rows[0].n} | com responsável: ${t.rows[0].comresp} | com prazo: ${t.rows[0].comdata}`);
  await c.end();
})().catch(e=>{console.error('ERRO:',e.message);process.exit(1);});
